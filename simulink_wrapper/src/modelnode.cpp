...
: Node("simulink_wrapper"){
    ...
    /*param_clbk_handle_verbosity = this->add_on_set_parameters_callback(
    std::bind(
      &SimulinkModelNode::param_clbk_verbosity,
      this,
      std::placeholders::_1));
    */

    // Create descriptor for the boolean parameter
    param_descriptor_verbosity.set__name("verbose");
    param_descriptor_verbosity.set__type(rcl_interfaces::msg::ParameterType::PARAMETER_BOOL);
    param_descriptor_verbosity.set__description("Set the code verbose.");
    param_descriptor_verbosity.set__additional_constraints("The parameter accepts only boolean value.");
    param_descriptor_verbosity.set__read_only(true);
    param_descriptor_verbosity.set__dynamic_typing(false);

    // Declare the boolean parameter with a default value of false
    this->declare_parameter("verbose", false, param_descriptor_verbosity);


    // SYMBOL_PREFIX
    // Create descriptor for the string parameter
    param_descriptor_symbolPrefix.set__name("symbolPrefix");
    param_descriptor_symbolPrefix.set__type(rcl_interfaces::msg::ParameterType::PARAMETER_STRING);
    param_descriptor_symbolPrefix.set__description("Set the symbol prefix.");
    param_descriptor_symbolPrefix.set__additional_constraints("The parameter accepts only strings: nameoftheproject_");
    param_descriptor_symbolPrefix.set__read_only(false);
    param_descriptor_symbolPrefix.set__dynamic_typing(false);


    this->declare_parameter<std::string>("symbolPrefix", "default_value", param_descriptor_symbolPrefix);
    this->get_parameter("symbolPrefix", symbolPrefix);
    RCLCPP_INFO(this->get_logger(), "Symbol Prefix: %s", symbolPrefix.c_str());

    // LIBRARY_NAME
    // Create descriptor for the boolean parameter
    param_descriptor_libraryName.set__name("libraryName");
    param_descriptor_libraryName.set__type(rcl_interfaces::msg::ParameterType::PARAMETER_STRING);
    param_descriptor_libraryName.set__description("Set the library name.");
    param_descriptor_libraryName.set__additional_constraints("The parameter accepts only strings.");
    param_descriptor_libraryName.set__read_only(false);
    param_descriptor_libraryName.set__dynamic_typing(false);

    // Declare the boolean parameter with a default value of false
    this->declare_parameter<std::string>("libraryName", "default_library_value", param_descriptor_libraryName);
    this->get_parameter("libraryName", libraryName);
...
/*rcl_interfaces::msg::SetParametersResult SimulinkModelNode::param_clbk_verbosity(
...
bool SimulinkModelNode::Initialize() {

    RCLCPP_INFO(this->get_logger(), "INITIALIZE FUNCTION: loaded dependencies on ROS2_node");

    library_handle_ = dlopen(libraryName.c_str(), RTLD_LAZY);
    ...
    symbol_prefix_ = symbolPrefix;

    if(verbose) RCLCPP_INFO(this->get_logger(), "Library %s loaded with symbol prefix %s", libraryName.c_str(), symbolPrefix.c_str());
